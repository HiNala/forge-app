...
- **Pipeline:** `stream_page_generation` (`app/services/orchestration/pipeline.py`) emits SSE: `intent` → `html.start` → `html.chunk` (per section fragment) → `html.complete` | `error`. Sections use `data-forge-section="{component}-{index}"` for splicing. `apply_plan_constraints` injects `form-vertical` for booking-style intents and `proposal-accept-decline` for proposals when missing. **Sentry:** `studio.pipeline` breadcrumbs at context ready, intent parsed, compose complete (agent vs template path), HTML validated, persist (create/update).
- **Component vocabulary:** Mission 00 described one-file-per-block HTML; shipped code uses **`app/services/orchestration/component_lib/catalog.py`** (named components) + **Jinja templates** under `component_lib/templates/`, with a few static HTML files under `components/` (e.g. `shell.html`).
