...
| **Auth** | First-party GlideDesign JWTs via `app/security/session_jwt.py`, bcrypt password hashing in `app/security/passwords.py`, refresh-token rotation in `auth_sessions`, and Google login identities in `oauth_identities`. |
...
**Planned / partial:** CSP on generated public HTML (inject via public runtime), full file-upload AV scan, CSRF on any future mutating route that relies on cookies instead of Bearer tokens.
