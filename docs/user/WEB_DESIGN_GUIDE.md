...
7. **Orphans** — A warning appears if a page (except the homepage) has no incoming edge. Link it from another page or draw an edge.
8. **Export** — **Export → Static site (single HTML file)** downloads a local preview bundle. Full Next.js zip and hosting are pipeline features.
