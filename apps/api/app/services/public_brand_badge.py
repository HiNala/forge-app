...
def _anon_ref(page_id: str) -> str:
    raw = hashlib.sha256(f"forge-badge:{page_id}".encode()).hexdigest()[:12]
    return raw
...
def inject_made_with_forge_badge(html: str, *, show: bool, page_id: str, forge_site_base: str) -> str:
    ...
    Skips if snippet already present.
    """
    ...
    badge = (
        ...
        ' background:rgba(15,23,42,0.78);color:#f8fafc;border:1px solid '
        'rgba(148,163,184,0.35);box-shadow:0 2px 8px rgba(0,0,0,0.15);">\n'
        '<span aria-hidden="true" style="opacity:0.9">✶</span> Made with Forge\n'
        "</a></div>"
    )
...
def forge_branding_visible_for_plan(plan: str | None) -> bool:
    """Show badge on trial + starter only."""
