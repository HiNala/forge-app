...
class StudioPipelineState:
    ...
    composed: Any
    # BP-01: optional four-layer payload + trace (Studio / saved run)
    product_brain_trace: list[dict[str, Any]] | None = None
    four_layer_output: dict[str, Any] | None = None


async def gather_studio_context_bundle(
    *,
    db: AsyncSession,
    ctx: TenantContext,
    user: User,
    ...
    """Brand load + context gather; replay chunks for SSE. None if org missing."""
    replay: list[bytes] = []
    brand_row = (
    ...
    return replay, org_row, bundle, brand_hint, brand_snapshot, primary, secondary


async def complete_studio_prep_from_gathered(
    *,
    db: AsyncSession,
    ctx: TenantContext,
    user: User,
    ...
    primary: str,
    secondary: str,
    page_intent_override: PageIntent | None = None,
    run_id: UUID | None = None,
    node_timings: dict[str, int] | None = None,
) -> tuple[list[bytes], StudioPipelineState | None]:
    """Compose intent + StudioPipelineState from an already gathered context bundle (BP-01)."""
    rid = run_id or uuid4()
    pipeline_started = time.perf_counter()
    nt = node_timings if node_timings is not None else {}
    composed = None

    ctx_block = bundle.to_prompt_context().strip()
    t_intent0 = time.perf_counter()
    if page_intent_override is not None:
        intent = apply_forced_workflow(page_intent_override, forced_workflow)
    else:
        try:
    ...
    nt["intent_parser"] = int((time.perf_counter() - t_intent0) * 1000)
    ...
    if intent.confidence < 0.85:
        ...
        graph_snapshot = {
            "phase": "clarify_pending",
            "candidates": clarify_cands[:3],
            "confidence": intent.confidence,
        }
...
async def prepare_studio_page_generation(
    *,
    ...
    if gathered is None:
        return [_sse("error", {"code": "not_found", "message": "Organization not found"})], None
    replay, org_row, bundle, brand_hint, brand_snapshot, primary, secondary = gathered

    return await complete_studio_prep_from_gathered(
        db=db,
        ctx=ctx,
        user=user,
        prompt=prompt,
        ...
        primary=primary,
        secondary=secondary,
        page_intent_override=page_intent_override,
        run_id=run_id,
...
async def stream_studio_page_generation_tail(state: StudioPipelineState) -> AsyncIterator[bytes]:
    ...
    bp_extras: dict[str, Any] = {}
    trace = getattr(state, "product_brain_trace", None)
    layers = getattr(state, "four_layer_output", None)
    if trace is not None:
        bp_extras["bp01_agent_calls"] = trace
    if layers is not None:
        bp_extras["bp01_four_layer"] = layers
    if bp_extras:
        review_report_blob = {**(review_report_blob or {}), **bp_extras}
    ...
    if existing is not None:
        ...
        try:
            pb_trace = getattr(state, "product_brain_trace", None)
            pb_four = getattr(state, "four_layer_output", None)
            await record_run(
                db,
                run_id=run_id,
                organization_id=ctx.organization_id,
                ...
                agent_calls=pb_trace,
                four_layer_output=pb_four,
            )
        except Exception:
    ...
    apply_memory = bool(prefs.forge_apply_memory)

    merged_fl: dict[str, Any] = {}
    flo = getattr(state, "four_layer_output", None)
    if isinstance(flo, dict):
        merged_fl = dict(flo)
    if not apply_memory:
        merged_fl.pop("memory_why", None)
    else:
        try:
            mem_why = await memory_explanation_bullets(
                db,
                organization_id=ctx.organization_id,
                user_id=user.id,
                apply_memory=True,
            ...
            mem_why = []
        if mem_why:
            merged_fl["memory_why"] = mem_why
    if merged_fl:
        four_layer_payload["four_layer"] = merged_fl
    yield _sse(
        "html.complete",
        four_layer_payload,
    )
