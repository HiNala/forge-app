...
async def forge_http_exception_handler(request: Request, exc: HTTPException) -> JSONResponse:
    ...
    if isinstance(detail, dict) and "code" in detail:
        ...
        extra_rest = {k: v for k, v in detail.items() if k not in ("code", "message", "extra")}
        return JSONResponse(
...
async def integrity_error_handler(request: Request, exc: IntegrityError) -> JSONResponse:
    ...
    return JSONResponse(
        status_code=500,
        content=(
            {"code": "internal_error", "message": "Database constraint error", "request_id": _request_id(request)}
            if is_prod
            else {
                "code": "internal_error",
                "message": str(exc),
...
async def payload_too_large_handler(request: Request, _exc: Exception) -> JSONResponse:
    return JSONResponse(
        status_code=413,
        content={
...
async def unhandled_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    return JSONResponse(
        status_code=500,
        content={"code": "internal_error", "message": str(exc), "request_id": rid},
    )
