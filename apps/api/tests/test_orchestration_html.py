...
from app.services.orchestration.html_validate import validate_generated_html, validate_section_html
...
<title>T</title></head><body><p>""" + ("x" * 80) + """</p></body></html>"""
...
<body><script>alert(1)</script>""" + ("y" * 80) + """</body></html>"""
    ok, reason = validate_generated_html(html)
    assert ok is False
