 ...
 * Without a GlideDesign session, `/dashboard` redirects to sign-in; we skip axe in that case.
 */
test("axe: /dashboard (authenticated only)", async ({ page }) => {
  await page.goto("/dashboard", { waitUntil: "load", timeout: 60_000 });
  if (page.url().includes("/signin")) {
    test.skip(true, "Not signed in — seed auth users and sign in for full app-shell E2E");
  }
